...
void pms5003_make_measurement(pms5003_config_t* inst, pms5003_measurement_t* reading)
{
    ...
    while (measurements_number < 1) // change to 1 to recieve data every second
    {
        int len = uart_read_bytes(UART_NUM_1, data, inst->uart_buffer_size, 1); // keep reading
        if (len > 0 && pms5003_process_data(len, data, &current_reading))
        {
            pms5003_add_measurements(reading, &current_reading);
            measurements_number++; // increment by one
        }
    }
    pms5003_div_measurement(reading, 1); // change to 1 to recieve data every second
...
void pms5003_print_measurement(pms5003_measurement_t* reading) // reads the size of the particles in the air
{
